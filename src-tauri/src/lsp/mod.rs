...
pub use status::{
    enrich_path, install_lsp_server, list_lsp_servers, LspInstallResult, LspServerStatus,
};

use client::LspClient;
use discover::{discover_servers, file_language, ServerSpec};

const MAX_REFS_PER_SYMBOL: usize = 24;
const DIAGNOSTIC_WAIT: Duration = Duration::from_millis(800);
const MAX_OPEN_FILES_PER_SERVER: usize = 200;
...
impl LspPool {
    pub fn start(
        ...
        on_progress(
            &format!("Starting {} language server(s)…", specs.len()),
            0,
            specs.len() as u32,
        );
        ...
        for (i, spec) in specs.into_iter().enumerate() {
            ...
            match spawn_server(&project_root, spec, Arc::clone(&diagnostics)) {
        ...
        std::thread::sleep(DIAGNOSTIC_WAIT);
        pool
    }
    ...
    fn open_files(
        ...
        let total = files.len().min(MAX_OPEN_FILES_PER_SERVER * self.servers.len().max(1));
        let mut opened = 0u32;

        for (rel, _) in files.iter().take(MAX_OPEN_FILES_PER_SERVER * 4) {
            ...
            for server in &mut self.servers {
                ...
                if server.opened.len() >= MAX_OPEN_FILES_PER_SERVER {
                    continue;
                }
    ...
    pub fn symbol_edges_for_file(
        ...
        for server in &self.servers {
            ...
            for sym in symbols.iter().take(40) {
                ...
                for loc in locs.into_iter().take(MAX_REFS_PER_SYMBOL) {
                    ...
                    // Find nearest symbol at or before this line
                    let target = by_line
                        .get(&target_line)
                        .copied()
                        .or_else(|| {
...
fn spawn_server(
    ...
    let on_diag = move |uri: String, diags: Vec<lsp_types::Diagnostic>| {
        ...
        // Replace previous diagnostics for this path
    ...
    client.initialize(&root_uri, spec.language.init_options())?;
    client.initialized()?;
